...
export function HeroSection({ onStartProject, onExploreServices }: HeroSectionProps) {
  ...
  // Background Parallax using raw units to prevent css % clashes on positioning
  const yBlob1 = useTransform(scrollYProgress, [0, 1], [0, 300])
  const yBlob2 = useTransform(scrollYProgress, [0, 1], [0, -250])
  const yBlob3 = useTransform(scrollYProgress, [0, 1], [0, 150])
  
  // Emoji Parallax
  const yEmoji1 = useTransform(scrollYProgress, [0, 1], [0, 180])
  const yEmoji2 = useTransform(scrollYProgress, [0, 1], [0, -150])
  const yEmoji3 = useTransform(scrollYProgress, [0, 1], [0, 130])
  const yEmoji4 = useTransform(scrollYProgress, [0, 1], [0, -100])
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        style={{ y: yBlob1, background: "radial-gradient(circle, #F2613F 0%, transparent 70%)" }}
        aria-hidden="true"
      />
      <motion.div
        className="absolute bottom-0 left-0 w-72 h-72 rounded-full blur-3xl opacity-20 pointer-events-none"
        style={{ y: yBlob2, background: "radial-gradient(circle, #0FA3B1 0%, transparent 70%)" }}
        aria-hidden="true"
      />
      <motion.div
        className="absolute top-1/2 left-1/4 w-48 h-48 rounded-full blur-3xl opacity-15 pointer-events-none"
        style={{ y: yBlob3, background: "radial-gradient(circle, #F5C046 0%, transparent 70%)" }}
        aria-hidden="true"
      />

      {/* Decorative floating emoji stickers */}
      <motion.div style={{ y: yEmoji1 }} className="absolute top-24 left-8 text-3xl opacity-70 hidden lg:block" aria-hidden="true">🎓</motion.div>
      <motion.div style={{ y: yEmoji2 }} className="absolute top-40 right-12 text-2xl opacity-60 hidden lg:block" aria-hidden="true">📚</motion.div>
      <motion.div style={{ y: yEmoji3 }} className="absolute bottom-20 left-16 text-2xl opacity-60 hidden lg:block" aria-hidden="true">💡</motion.div>
      <motion.div style={{ y: yEmoji4 }} className="absolute bottom-32 right-8 text-3xl opacity-70 hidden lg:block" aria-hidden="true">🚀</motion.div>
          ...
          >
            ...
            >
              <div className="relative w-full aspect-square max-w-sm mx-auto lg:max-w-none">
                {/* Main card */}
                <div className="rounded-2xl bg-white border border-border shadow-2xl p-6 relative z-10 transition-transform duration-500 hover:scale-[1.02]" style={{ borderColor: "rgba(242,97,63,0.15)" }}>
                ...
                {floatingCards.map((card, i) => {
                  ...
                  return (
                    <motion.div
                      ...
                      dragElastic={0.2}
                      className={`${positions[i]} z-20 cursor-grab active:cursor-grabbing bg-white border border-border rounded-xl shadow-xl px-3 py-2.5 flex items-center gap-2.5 hover:scale-105 transition-transform`}
